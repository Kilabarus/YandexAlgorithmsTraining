    ...
    internal class Program
    {
        ...
        class FamilyTree
        {
            ...
            public string InfixTraversal()
            {
                ...
                InfixTraversal(root, depths, 0);

                StringBuilder sb = new();

                foreach (var parent in depths.OrderBy(p => p.Key))
                {
            ...
            private void InfixTraversal(Node node, Dictionary<string, int> depths, int currDepth)
            {
                if (node.Children.Count > 0)
                {
                    foreach (var childNode in node.Children)
                    {
                        InfixTraversal(childNode, depths, currDepth + 1);
                    }
                }
                
                depths.Add(node.Value, currDepth);
            }
        }
